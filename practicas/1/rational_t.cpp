...
{if (fabs(value()-r.value())<=precision) return true;
	else return false;
}
...
{if (fabs(value()-r.value())<( (-1)*precision)) return true;  // if (fabs(r.value()-value())>precision) return true;
	else return false;
}
